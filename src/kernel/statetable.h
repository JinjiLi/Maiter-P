...
#pragma pack(pop)
    ...
    public:
        struct Iterator : public TypedTableIterator<K, V1, V2, V3> {//2

            Iterator(StateTable<K, V1, V2, V3>& parent, int num_threads, bool bfilter) : pos(-1), parent_(parent), num_threads(num_threads) {
        ...
        struct ScheduledIterator : public TypedTableIterator<K, V1, V2, V3> {//3

            ScheduledIterator(StateTable<K, V1, V2, V3>& parent, int num_threads, bool bfilter) : pos(-1), parent_(parent), num_threads(num_threads) {
        ...
        struct EntirePassIterator : public TypedTableIterator<K, V1, V2, V3>, public LocalTableIterator<K, V2> {//4
        ...
        TableIterator *get_iterator(int num_threads, bool bfilter) {
            ...
            Iterator* iter = new Iterator(*this, num_threads, bfilter);
            ...
            while (iter->b_no_change) {
                ...
                iter = new Iterator(*this, num_threads, bfilter);
        ...
        TableIterator *schedule_iterator(int num_threads, bool bfilter) {

            if (terminated_) return NULL;
          
            sleep(0.1);

            ScheduledIterator* iter = new ScheduledIterator(*this, num_threads, bfilter);
            while (iter->b_no_change) {
                ...
                iter = new ScheduledIterator(*this, num_threads, bfilter);
    ...
    template <class K, class V1, class V2, class V3>
    void StateTable<K, V1, V2, V3>::serializeToSnapshot(long* updates, double* totalF2) {
        total_curr = 0;
